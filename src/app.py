...
def editUserMYSQL():
    ...
    return jsonify({'message': 'USUARIO EDITADO CON EXITO'})
...
def addUserMONGO():
    mongo.db.userdb.insert_one(request.get_json())
    return jsonify({'message': 'USUARIO AÑADIDO CON EXITO'})

@app.route('/getUsersMONGO', methods=['GET'])
def getUsersMONGO():
    usuarios = mongo.db.userdb.find()
...
def editUserMONGO():
    ...
    mongo.db.userdb.update_one({"nombre":data['nombre']}, {'$set':data})
    return jsonify({'usuarios': 'USUARIO MODIFICADO CORRECTAMENTE' })


@app.route('/deleteUserMONGO', methods=['DELETE'])
def deleteUserMONGO():
    data = request.get_json()
    mongo.db.userdb.delete_one({"nombre":data['nombre']})
